...
public class ChatPresenter implements Observer {
    ...
    public void getMessage(@Nullable TIMMessage message){
        if (!isGetingMessage){
            ...
            TIMConversationExt timConversationExt = new TIMConversationExt(conversation);
           // timConversationExt.saveMessage()
    ...
    public void saveMessage(TIMMessage message){
        TIMConversationExt timConversationExt = new TIMConversationExt(conversation);

        List<TIMMessage> temp = new ArrayList<>();
        temp.add(message);
        timConversationExt.importMsg(temp);
      //  timConversationExt.saveMessage(message,"100",false);
    }
